#!/usr/bin/env python3
"""Generate the 29-case C4 GPU density-fitting candidate ladder."""
...
SOURCE_SUITE_ID = "si_ge_h_cl_ladder_v1"
SUITE_ID = "gpu_c4_density_fit_ladder_v1"
MINAO_SUITE_ID = "gpu_c4_density_fit_minao_ladder_v1"
CATEGORY = "gpu_c4_density_fit"
...
def generate(
    ...
    *,
    ...
    case_suffix: str = "_c4_density_fit",
    ...
    for row in source["cases"]:
        ...
        if actual != expected:
            raise ValueError(f"{base_case_id} baseline settings {actual} != {expected}.")
    ...
    suite = {
        ...
        "suite_id": suite_id,
        "description": (
            "C4 full 29-case GPU ladder for the conditional density-fitting "
            "candidate"
            + (" with an explicit shared MINAO initial density. " if explicit_minao else ". ")
            + "Grid 5, VV10 grid 5, and all scientific inputs remain "
            "unchanged. Direct SCF remains the fallback protocol."
        ),
...
def main() -> int:
    ...
    minao_suite_path, minao_written = generate(
        ...
        case_suffix="_c4_density_fit_minao",
        explicit_minao=True,
    )
    print(
