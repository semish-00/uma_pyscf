#!/usr/bin/env bash
#SBATCH --job-name=uma-overfit-50-v2
...
RUN_ROOT="${RUN_ROOT:-/lustre/user140002/runs/uma/overfit_engineering_50_v2/${SLURM_JOB_ID}}"
CONFIG="${CONFIG:-$REPO_ROOT/configs/finetune/engineering_50_overfit_v2.yaml}"

mkdir -p /lustre/user140002/runs/slurm "$RUN_ROOT"
sha256sum "$CONFIG" "$REPO_ROOT/configs/finetune/engineering_50_overfit_v1.yaml" \
  "$REPO_ROOT/configs/finetune/data/engineering_50_omol_ef_v1.yaml" \
...
srun \
  ...
  bash -lc "
    ...
    '$PYTHON_ROOT/bin/fairchem' -c '$CONFIG'
  "
