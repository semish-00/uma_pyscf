#!/usr/bin/env bash
# Relax six T0-only parents, run short blind base-UMA NVT trajectories, and
# freeze the 76-record finite-temperature T0 source tranche.

#SBATCH --job-name=uma-t0-md
...
#SBATCH --output=/lustre/user140002/logs/uma-t0-md-%j.out
#SBATCH --error=/lustre/user140002/logs/uma-t0-md-%j.err
...
SEED_CONFIG="$REPO_ROOT/configs/sampling/t0_dimer_parent_seeds_v1.yaml"
MD_CONFIG="$REPO_ROOT/configs/sampling/t0_dimer_uma_md_v1.yaml"
PORTFOLIO_CONFIG="$REPO_ROOT/configs/sampling/t0_dimer_md_portfolio_76_v1.yaml"
RUN_ROOT="${RUN_ROOT:-/lustre/user140002/runs/test/t0_dimer_md_v1/${SLURM_JOB_ID}}"
...
srun \
  ...
  bash -lc "
    ...
    '$PYTHON' validation/uma_sampling/run_langevin_md.py \
      --config '$MD_CONFIG' \
      --manifest '$RUN_ROOT/seeds/t0_dimer_parent_seeds_v1_candidates.json' \
...
sha256sum \
  ...
  "$RUN_ROOT/candidates/t0_dimer_uma_md_v1_pool_candidates.json" \
  "$RUN_ROOT/candidates/t0_dimer_uma_md_v1_pool_geometry_qc.json" \
  "$RUN_ROOT/portfolio/t0_dimer_md_76_v1_candidates.json" \
  "$RUN_ROOT/portfolio/t0_dimer_md_76_v1_report.json" \
  > "$RUN_ROOT/provenance/outputs.sha256"

echo "T0 finite-temperature candidate artifacts: $RUN_ROOT"
