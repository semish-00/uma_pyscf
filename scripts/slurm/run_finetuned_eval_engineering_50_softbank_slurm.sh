#!/usr/bin/env bash
#SBATCH --job-name=uma-ft-eval-50
...
: "${CHECKPOINT:?Set CHECKPOINT to a final inference_ckpt.pt}"
...
RUN_ROOT="${RUN_ROOT:-/lustre/user140002/runs/uma/finetuned_eval_engineering_50_v1/${SLURM_JOB_ID}}"
...
srun \
  ...
  bash -lc "
    ...
    '$PYTHON' -m uma_pyscf.cli.main evaluate-uma \
      --config '$REPO_ROOT/configs/evaluation/engineering_50_finetuned_200step_v1.yaml' \
