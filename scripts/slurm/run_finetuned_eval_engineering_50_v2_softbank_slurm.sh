#!/usr/bin/env bash
#SBATCH --job-name=uma-ft10k-eval-50
...
: "${CHECKPOINT:?Set CHECKPOINT to the 10,000-step final inference_ckpt.pt}"
...
RUN_ROOT="${RUN_ROOT:-/lustre/user140002/runs/uma/finetuned_eval_engineering_50_v2/${SLURM_JOB_ID}}"
...
srun \
  ...
  bash -lc "
    ...
    '$PYTHON' -m uma_pyscf.cli.main evaluate-uma \
      --config '$REPO_ROOT/configs/evaluation/engineering_50_finetuned_10000step_v1.yaml' \
