#!/usr/bin/env bash
# Freeze the two source-stratified random-control shell manifests.

#SBATCH --job-name=c0-random-prep
#SBATCH --partition=140-partition
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=1
#SBATCH --time=00:10:00
#SBATCH --output=/lustre/user140002/logs/c0-random-prep-%j.out
#SBATCH --error=/lustre/user140002/logs/c0-random-prep-%j.err
...
RUN_ROOT="${RUN_ROOT:-/lustre/user140002/runs/calibration/c0_random_shell_v1/${SLURM_JOB_ID}}"
SCAN_CONFIG="$REPO_ROOT/configs/sampling/c0_random_scan_shell_v1.yaml"
DISTORT_CONFIG="$REPO_ROOT/configs/sampling/c0_random_distortion_shell_v1.yaml"
...
sha256sum "$SCAN_CONFIG" "$DISTORT_CONFIG" \
  "$REPO_ROOT"/configs/sampling/structures/c0_random_shell_seeds_v1/*.xyz \
...
srun --container-image="$CONTAINER_IMAGE" --container-mounts=/lustre:/lustre \
  ...
  "
sha256sum "$RUN_ROOT/scan/c0_random_scan_shell_v1_candidates.json" \
  "$RUN_ROOT/scan/c0_random_scan_shell_v1_geometry_qc.json" \
  "$RUN_ROOT/distortion/c0_random_distortion_shell_v1_candidates.json" \
  "$RUN_ROOT/distortion/c0_random_distortion_shell_v1_geometry_qc.json" \
