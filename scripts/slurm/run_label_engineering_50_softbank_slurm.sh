#!/bin/bash
# 50-candidate engineering run: sample -> label -> resume -> production QC.

#SBATCH --job-name=uma-label-e50
...
SAMPLE_CONFIG="${SAMPLE_CONFIG:-$REPO_ROOT/configs/sampling/engineering_50_v1.yaml}"
...
RUN_ROOT="${RUN_ROOT:-/lustre/user140002/runs/label/engineering_50_v1/$SLURM_JOB_ID}"
...
srun \
  ...
  bash -lc '
    ...
    MANIFEST="$RUN_ROOT/input/engineering_50_v1_candidates.json"
    ...
    python3 -m uma_pyscf.cli.main label \
      ...
      --manifest "$MANIFEST" \
      --output-dir "$RUN_ROOT/label"
    cp "$RUN_ROOT/label/summary.json" "$RUN_ROOT/label/summary_first_pass.json"
    python3 -m uma_pyscf.cli.main label \
      --config "$DFT_CONFIG" \
      --manifest "$MANIFEST" \
      --output-dir "$RUN_ROOT/label"
    cp "$RUN_ROOT/label/summary.json" "$RUN_ROOT/label/summary_resume_pass.json"
...
echo "Engineering-50 artifacts: $RUN_ROOT"
